...
/* GET SUMMARY REPORT */
router.get("/summary", requireRole(["teacher", "student"]), (req, res) => {
  ...
  const query = `SELECT
      1 AS totalStudents,
      1 AS totalCourses,
      (SELECT COUNT(*) FROM attendance WHERE student_id = ?) AS totalAttendance,
      (SELECT COUNT(*) FROM marks WHERE student_id = ?) AS totalMarks,
      (SELECT ROUND(COALESCE(AVG(CASE WHEN status = 'Present' THEN 1 ELSE 0 END), 0) * 100, 2) FROM attendance WHERE student_id = ?) AS attendanceRate,
      (SELECT ROUND(COALESCE(AVG(score / NULLIF(max_score, 0)), 0) * 100, 2) FROM marks WHERE student_id = ?) AS averageScore`;

  db.query(query, [studentId, studentId, studentId, studentId], (err, result) => {
    if (err) return res.status(500).json(err);
    res.json(result[0]);
  });
});
...
router.get("/student/:id", (req, res) => {
  ...
  db.query(
    ...
    (err, studentResult) => {
      if (err) return res.status(500).json(err);
      if (!studentResult.length) return res.status(404).json({ message: "Student not found" });
